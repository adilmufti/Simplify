...
looping = len(arguments)


def split(word):
    return [char for char in word]

def simplifyFormulas(expression):
    expression = expression.lower()
    ...
    if '∨' in expression and len(expression) >= 7:
        ...
        lst = split(right)
        lst2 = split(left)
        if lst[0] == '(' and lst[2] == '∧' and lst[4] == ')' and lst[1] != lst[3] and lst2[-1] == lst[1] and lst2[-1] != lst[3]:
            del lst[0]
            del lst[0]
            del lst[0]
            del lst[0]
...
#so split the expression here and then you do index like below. That may not work so maybe you can change expression into
...
def removeBiImp(expression):
    if '↔' in expression:
        (left,right) = expression.split('↔',1)  #Split from first occurence of <>
...
logic = str(input("THIS IS THE USER'S INPUT: "))
getArguments(logic)
removeBiImp(logic)
finished = False
while finished == False:
    len1 = len(logic)
    logic = simplifyFormulas(logic)
    len2 = len(logic)
    if len1 - len2 == 0:
        finished = True
print(logic)
